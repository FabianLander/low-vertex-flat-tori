 ...
 * itself. Over every non-adjacent cell pair (vertex/edge/face that share no
 * vertex) it measures the 3D gap, normalizes by the torus's linear size
 * L = √(total area) so it is scale-free, and returns the SMALLEST such gap and
 * which pair realizes it.
 *
 * This is a *value* map, not an energy — `certify` records it, the embedded
 * `Region` reports it, the search scripts log it. (A gap ≥ ε means the cell-margin
 * energy is zero; the gate itself is `isEmbedded`, the topological truth, not
 * `margin > 0`.)
 *
 * The six pair types and their metrics:
 *   vertex–vertex : point→point        vertex–edge : point→segment
 *   vertex–face   : point→triangle     edge–edge   : each midpoint→other segment
 *   edge–face     : midpoint→triangle  face–face   : triangle→triangle
 *
...
// --- per-pair-type Euclidean gaps (normalization happens in the caller) ---
...
/** Edge–edge: midpoint of each edge to the other segment (two measurements). */
 ...
 * independent of any energy's ε/weight.
 */
