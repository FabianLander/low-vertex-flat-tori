 ...
 * paired with a `Triangulation` (the combinatorics). Each row is a different point in
...
export function paperFromRow(torus: Triangulation, row: ArrayLike<number>): PaperTorus {
...
export function parseEmbeddings(csvText: string, torus: Triangulation): PaperTorus[] {
  const n = torus.vertexCount * 3;
