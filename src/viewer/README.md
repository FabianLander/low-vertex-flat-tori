...
- `materials.ts` — surface (plain/grid), crease, vertex material primitives + `paperMaterials`
  (the graph-paper theme used by the renders).
...
Impure: the three.js + DOM boundary. Consumes `mesh/` + `topology/`; the path-traced
harness (`render/`: `Studio` / `stage` / `controls`) sits above. The appearance group is
