...
Impure: the three.js boundary. Consumes `topology/` (the `Triangulation` + `develop`)
and `geometry/` (the `Vec3` tuple); the `PaperTorus` boundary bundle appears only at
`obj`. The subject that assembles these parts lives in `viewer/`.
