...
public class ShapeCanvas extends ScaleCanvas implements MouseListener, MouseMotionListener, KeyListener {
    ...
    /**This routine changes the normalization for the golden valley
       so that it matches what is in the paper. currently disabled
       because it is too slow when the goodpath routine is turned on.*/
    
    public Complex specialTransform() {
	Torus T=M.C.getTorus();
	int m=M.C.CHOICE.mode;
	Complex z1=Tiling.lattice2(T);
	Complex z2=Tiling.intrinsicShape(T);
	Complex z3=new Complex(0,4*z2.y);
	Complex z4=Complex.divide(z3,z1); //this is the correcting factor
	return z4;
    }
	...
	Complex t1=Tiling.lattice1(T);
	Complex t2=Tiling.lattice2(T);
	t0=Complex.times(t0,u);
	t1=Complex.times(t1,u);
	t2=Complex.times(t2,u);
