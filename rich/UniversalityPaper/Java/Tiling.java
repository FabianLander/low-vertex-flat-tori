...
public class Tiling {
	...
	int[] s={11,5,4,14,3,1,0,6};
	int[] ss = {0, 1, 13, 2, 14, 15, 5, 10, 11, 4, 12, 6, 7, 9, 3, 8};
	Complex[][] Z=fundamentalDomainRaw(T);
	Complex w=Complex.plus(Z[1][1],Z[1][2]);
	w=w.scale(.5);
	for(int i=0;i<16;++i) {
	    for(int j=0;j<3;++j) {
		int l=Z[i][j].label;
		Z[i][j]=Complex.minus(Z[i][j],w);
		Z[i][j].label=l;
	    }
	}
	
	Complex[][] W=new Complex[16][3];
	for(int i=0;i<8;++i) {
	    W[i]=Z[ss[s[i]]];
	    W[15-i]=negate(Z[ss[s[i]]]);
	}
	Complex u=lattice1(T);
	
	for(int i=0;i<3;++i) {
	    int l=W[7][i].label;
	    W[7][i]=Complex.minus(W[7][i],u);
	    W[7][i].label=l;
	}
	
	for(int i=0;i<3;++i) {
	    int l=W[8][i].label;
	    W[8][i]=Complex.plus(W[8][i],u);
	    W[8][i].label=l;
	}
    ...
    public static Complex[][] fundamentalDomainRaw(Torus T) {
	int[][] F=TriangulationCombinatorics.tiling();
