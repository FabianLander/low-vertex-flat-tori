...
const CONFIG = {
  ...
  background: 0xeef0f3,
  planeRoughness: 0.95,
  planeDistance: 1.5,          // how far behind the torus (× scene radius)
  planeSize: 40,               // huge so it fills the frame
...
const edgeMaterial = creaseEdgeMaterial(CONFIG.torusColor);   // match the paper color
...
const wall = backWall({ color: CONFIG.wallColor, width: 40, height: 40, roughness: CONFIG.planeRoughness });
...
function setSubject(): void {
  ...
  studio.frame(t, { direction: new THREE.Vector3(0, 0, 1) });
