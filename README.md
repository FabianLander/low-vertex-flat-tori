...
src/core/            THE PURE, HEADLESS CORE — no three.js, no DOM; every folder runs under tsx.
  ...
  Dependency-ordered, each layer using only the ones below:
    ...
    constraints/       the CLOSED conditions {g=0} you project onto (+ types.ts: Constraint = Fn): flat
                       (emits V−1 rows), collinear, modulus — the point/line/circle × Teichmüller/moduli
                       grid (pinTeichmuller/pinModuli × point/verticalLine/circle; named: fixedModulus, modulusWall).
