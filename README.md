...
2. Run `npm run compute-markings` to (re)generate the per-census `*.markings.generated.ts` files
   (the expensive harmonic-layout + min-cut pass, run once, committed; 1:1 with the data files).
