...
src/core/            THE PURE, HEADLESS CORE — no three.js, no DOM; every folder runs under tsx.
  ...
  triangulations/     the specific triangulations we study, as DATA — scales to many:
    eightVertex.ts       the 7 V=8 triangle lists (TriangulationData, ids v8-1..v8-7); add nineVertex.ts later
    markings.generated.ts  the precomputed markings keyed by id — GENERATED by `npm run compute-markings`
    index.ts             the registry: ALL_TORI, RICH, byId('v8-7') — joins data ⊕ markings via
...
2. an **`attempt` recipe** — `(seed) => Certificate | null`: run `project`/`minimize` on the seed and
...
1. Add a `{ id, triangles }` entry (`TriangulationData`) to a data file
   (`src/core/triangulations/eightVertex.ts`, or a new per-vertex-count file like `nineVertex.ts`).
2. Run `npm run compute-markings` to (re)generate `markings.generated.ts` — the precomputed marking
   table (the expensive harmonic-layout + min-cut pass, run once, committed).
3. Add the data array to `DATA` in `src/core/triangulations/index.ts`.
