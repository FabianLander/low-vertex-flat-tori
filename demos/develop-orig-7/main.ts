...
const data = tori.map((p) => {
  ...
  return { net, base, bluePath, redPath, v1, tau, U, cU };
});
...
function fitDeveloped(corners: V2[][], cx: number, cy: number, cw: number, ch: number, m: number): Fit {
  let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
  for (const tri of corners) for (const [x, y] of tri) { if (x < minX) minX = x; if (x > maxX) maxX = x; if (y < minY) minY = y; if (y > maxY) maxY = y; }
  const scale = Math.min((cw - 2 * m) / (maxX - minX || 1), (ch - 2 * m) / (maxY - minY || 1));
  const ox = cx + (cw - scale * (minX + maxX)) / 2;
  const oy = cy + (ch + scale * (minY + maxY)) / 2;
  return { sx: (p) => ox + scale * p[0], sy: (p) => oy - scale * p[1], scale };
}
...
function drawCell(d: typeof data[number], cx: number, cy: number, cw: number, ch: number): void {
  const f = fitDeveloped(d.net.corners, cx, cy, cw, ch, 36);
